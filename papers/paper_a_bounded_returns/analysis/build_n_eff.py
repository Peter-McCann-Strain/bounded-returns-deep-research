...
# B raised 2000 -> 20000 (2026-08-11). At B=2000 the smallest attainable two-sided p under
# (count+1)/(n+1) is 2/2001 = 0.001, so the seven strongly-significant dimensions were floored
# by the resample size rather than by the data, and the worst-case p wobbled ~0.0345-0.0381
# across seeds. Raising B does not change the estimand, only Monte Carlo precision.
N_BOOT = 20000
SEED = 20260611  # prereg seed (E3)

def _cluster_index_map(w):
    """Row positions grouped by query_id, so a resample can repeat a cluster's rows
    according to its draw multiplicity."""
    q = w.index.get_level_values("query_id").to_numpy()
    order = np.argsort(q, kind="stable")
    q_sorted = q[order]
    uniq, starts = np.unique(q_sorted, return_index=True)
    ends = np.append(starts[1:], len(q_sorted))
    return {u: order[s:e] for u, s, e in zip(uniq, starts, ends)}


def _resample_clusters(w, idx_map, chosen):
    """Select the drawn clusters' rows, preserving draw multiplicity.

    A membership test (np.isin, or `in` against a set) contributes a cluster drawn m>1
    times only ONCE. That degrades the cluster bootstrap into an unrescaled subsample
    WITHOUT replacement of mean size n*(1-(1-1/n)^n) ~ 63% of n, so the replicate variance
    is sigma^2(1/0.63n - 1/n) instead of sigma^2/n -- about 58% of the correct value, which
    shrinks every CI and p-value built on it. Repeating the row blocks is the fix.
    """
    parts = [idx_map[q] for q in chosen if q in idx_map]
    if not parts:
        return w.iloc[[]]
    return w.iloc[np.concatenate(parts)]
...
def _excess_boot_pvals(per_dim_groups):
    ...
    idx_maps = {d: _cluster_index_map(w) for d, w in wides.items()}
    ...
    for it in range(N_BOOT):
        ...
        for d in dims:
            w = wides[d]
            sel = _resample_clusters(w, idx_maps[d], chosen)
    ...
    for d in dims:
        for metric in ("phi", "ac1"):
            ...
            # (count+1)/(n+1), not count/n: the naive ratio reports p=0.0 exactly whenever no
            # replicate crosses zero, which is not an attainable p-value -- the resample can
            # only bound p from below at 1/(n+1). Davison & Hinkley (1997), eq. 4.10.
            nb = len(bd)
            cnt_le0 = int((bd <= 0).sum()); cnt_ge0 = int((bd >= 0).sum())
            p_one = (min(cnt_le0, cnt_ge0) + 1.0) / (nb + 1.0)
            pvals[(d, metric)] = float(min(1.0, 2.0 * p_one))
    return pvals
...
def _overall_neff_ci(df, n_boot=N_BOOT, seed=SEED):
    ...
    idx_map = _cluster_index_map(w)
    pairs = list(itertools.combinations(PANEL, 2))
    boot = np.empty(n_boot)
    for it in range(n_boot):
        chosen = all_q[rng.integers(0, len(all_q), len(all_q))]
        sel = _resample_clusters(w, idx_map, chosen)
...
if DRY_RUN:
    ...
    def _merge(dst, src):
        """Recursively overlay src onto dst, preserving dst keys src does not produce.

        `cn["n_eff"] = out` silently destroyed every n_eff key written by a DIFFERENT
        script -- n_eff.within_openai.* (cited at main.tex:1217),
        n_eff.participation_ratio_overall_scores.* (cited at main.tex:1206-1209) and
        n_eff.diagnostics.condorcet_null: 86 leaves, gone on every run, with no error.
        This is the store-degradation defect logged as E4. Overlay, never replace.
        """
    ...
    _merge(cn.setdefault("n_eff", {}), out)
