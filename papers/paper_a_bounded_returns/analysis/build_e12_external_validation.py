...
commit 0a80ba6 to papers/paper_a_bounded_returns/analysis/.  This script targets
the CORRECT post-move path; it does NOT use the stale papers/paper_a_bounded_returns
...
Usage:
    ...
    python papers/paper_a_bounded_returns/analysis/build_e12_external_validation.py --dry-run

    # land canonical['external_validation_e12'] (requires complete E12 verdicts):
    python papers/paper_a_bounded_returns/analysis/build_e12_external_validation.py

    # land even if verdicts are incomplete (records coverage; status=partial):
    python papers/paper_a_bounded_returns/analysis/build_e12_external_validation.py --allow-partial
"""
...
# parents[3]: papers/paper_a_bounded_returns/analysis/<this> -> repo root.
_REPO_ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(_REPO_ROOT))

from deep_research.config import MODELS  # noqa: E402


def _merge(dst, src):
    """Overlay src onto dst, preserving dst keys src does not produce.
    Assigning a whole subtree destroys sibling keys written by OTHER scripts. That defect
    silently removed store leaves three times in one session (86 + 49 + 75). Overlay, never
    replace. If a script ever legitimately needs to REMOVE a key, do it explicitly."""
...
# Deployment names are provisioning-specific; record whatever generated the run.
GENERATION_DEPLOYMENT = getattr(MODELS.get("gpt-4o"), "deployment", "unknown")

# CORRECT post-move canonical location (NOT papers/paper_a_bounded_returns/analysis).
...
def main() -> int:
    ...
    block = {
        ...
        "generation_deployment": GENERATION_DEPLOYMENT,
    ...
    _merge(canon.setdefault("external_validation_e12", {}), block)
