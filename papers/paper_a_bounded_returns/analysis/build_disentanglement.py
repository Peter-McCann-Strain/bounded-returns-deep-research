...
def runs_metric(pat, col, qs=None):
    """Mean of `col` over SUCCESSFUL runs, optionally on a matched query set.

    disentangle_matched_p1 completed 29/90 queries; .mean() skipping NaN divided a
    29-query numerator by a 90-query base_p0 denominator. base_p0 costs 24.3% more
    on those 29 queries, so quoting the clamped and unclamped multiples together
    requires the matched basis (2.54x / 9.90x, not 3.16x / 12.07x).
    """
    d = RUNS[(RUNS.pattern.astype(str) == pat) & (RUNS.status == "success")]
    if qs is not None:
        d = d[d.query_id.isin(qs)]
...
if costcol:
    ...
    _mp1 = runs_metric("disentangle_matched_p1", costcol)
    budget["matched_p1"] = round(_mp1, 4) if _mp1 else None

    # Matched-basis block. disentangle_matched_p1 completed 29/90 queries, and base_p0
    # costs 24.3% more on those 29, so the clamped and unclamped multiples must be
    # quoted on the SAME query set. main.tex cites p1_over_p0_ratio_q29 (9.9x) and
    # matched_p1_over_p0_ratio_q29 (2.54x); this block regenerates them, so a re-run
    # cannot silently drop the values the paper depends on.
    _Q = set(RUNS[(RUNS.pattern.astype(str) == "disentangle_matched_p1")
                  & (RUNS.status == "success")].query_id)
    _p0q = runs_metric("base_p0", costcol, _Q)
    _p1q = runs_metric("base_p1", costcol, _Q)
    _mpq = runs_metric("disentangle_matched_p1", costcol, _Q)
    budget["matched_basis_n"] = len(_Q)
    budget["base_p0_q29"] = round(_p0q, 4) if _p0q else None
    budget["base_p1_q29"] = round(_p1q, 4) if _p1q else None
    budget["matched_p1_q29"] = round(_mpq, 4) if _mpq else None
    budget["p1_over_p0_ratio_q29"] = round(_p1q / _p0q, 2) if _p0q and _p1q else None
    budget["matched_p1_over_p0_ratio_q29"] = round(_mpq / _p0q, 2) if _p0q and _mpq else None
    budget["_basis_note"] = (
        "The 90-query means divided a 29-query matched_p1 numerator by a 90-query "
        "base_p0 denominator. base_p0 costs 24.3% more on the 29 probe queries, so "
        "both ratios must be quoted on the matched basis: 9.90x unclamped, 2.54x clamped.")
...
def _merge(dst, src):
    ...
    Assigning a whole subtree destroys sibling keys written by OTHER scripts."""
...
if "--write" in sys.argv:
    ...
    _merge(cn.setdefault("disentanglement", {}), res)
