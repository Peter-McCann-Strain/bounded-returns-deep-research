...
D = json.load(open(f"{ROOT}/papers/paper_a_bounded_returns/analysis/canonical_numbers.json"))["disentanglement"]
...
# Ratios must be quoted on the MATCHED 29-query basis (see canonical_numbers.json
# disentanglement.budget_spread._basis_note): the old 12.07x/3.16x labels divided a
# 29-query matched_p1 numerator by a 90-query base_p0 denominator, and P0 costs 24.3%
# more on the 29 probe queries. Both erred the same way, depicting a LARGER compute
# manipulation than was performed -- i.e. a gentler-looking clamp than the real one.
labels = ["full budget\n(~9.9$\\times$ P0)", "matched budget\n(~2.5$\\times$ P0)"]
...
plt.tight_layout()
for ext in ("pdf", "png"):
    plt.savefig(f"{ROOT}/papers/paper_a_bounded_returns/figures/fig_disentanglement.{ext}", dpi=200, bbox_inches="tight")
