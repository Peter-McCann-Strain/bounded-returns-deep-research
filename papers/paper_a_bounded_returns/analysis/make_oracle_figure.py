...
o = json.load(open(f"{ROOT}/papers/paper_a_bounded_returns/analysis/canonical_numbers.json"))["oracle"]
...
# Derive the limits from the CIs actually plotted rather than hardcoding them. The old
# (-0.06, 0.29) was set by checking only the UPPER extreme: logical_coherence's lower bound
# is -0.0759, so 15% of that interval fell outside the axes and matplotlib's default
# clip_on=True dropped its end cap entirely -- while the caption asserts the whiskers ARE
# the 95% CIs. A hardcoded limit cannot survive the data changing; this one can.
_los = [c[0] for _, _, c in rows]; _his = [c[1] for _, _, c in rows]
_pad = 0.06 * (max(_his) - min(_los))
axA.set_xlim(min(_los) - _pad, max(_his) + _pad)
...
plt.tight_layout()
for ext in ("pdf", "png"):
    plt.savefig(f"{ROOT}/papers/paper_a_bounded_returns/figures/fig_oracle.{ext}", dpi=300, bbox_inches="tight")
