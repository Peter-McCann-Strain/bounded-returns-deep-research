...
Usage:
    ...
    python papers/paper_a_bounded_returns/analysis/build_drb_race_by_family.py

    # land the cells into canonical_numbers.json (atomic; merge-only)
    python papers/paper_a_bounded_returns/analysis/build_drb_race_by_family.py --write
"""
...
def _merge_write(by_family_cells: dict, corr_updates: dict) -> None:
    ...
    for judge_label, res in corr_updates.items():
        drc[judge_label] = res
    jvh["by_family"] = bf
    _merge(cn.setdefault("judge_vs_human", {}), jvh)
    tmp = str(CN_PATH) + ".tmp"
