...
import json, os, warnings, sys, os
import numpy as np
...
d=json.load(open(f"{ROOT}/papers/paper_a_bounded_returns/analysis/canonical_numbers.json"))
...
plt.tight_layout()
for ext in ("pdf","png"):
    plt.savefig(f"{ROOT}/papers/paper_a_bounded_returns/figures/fig_cost.{ext}",dpi=300,bbox_inches="tight")
...
# Atomic write: a bare json.dump(open(...,"w")) truncates the store before
# writing, so an exception mid-dump leaves canonical_numbers.json destroyed.
_tmp = f"{f"{ROOT}/papers/paper_a_bounded_returns/analysis/canonical_numbers.json"}.tmp"
with open(_tmp, "w") as _f:
    json.dump(cn, _f,indent=1)
os.replace(_tmp, f"{ROOT}/papers/paper_a_bounded_returns/analysis/canonical_numbers.json")
