...
ROOT=str(PROJECT_ROOT); A=f"{ROOT}/data/analysis"
...
feat=pd.DataFrame({
 ...
 # COUNTS, not just rates: the rate alone cannot distinguish "the rubric ignores grounding"
 # from "the rubric rewards grounded volume and barely penalises placeholders".
 "n_grounded": g.apply(lambda d:d.category.isin(["academic","real_url"]).sum()),
 "n_placeholder": g.apply(lambda d:(d.category=="placeholder").sum()),
...
out={"n_reports":int(len(df)),"r2":round(float(m.rsquared),4),"r2_adj":round(float(m.rsquared_adj),4),
     ...
     "corr_cq_density_note":"'density' here is log_cit (the log citation count used as the regressor "
               "throughout this model), NOT the untransformed count. Reported separately below because "
               "the paper's prose previously described this figure as the raw count, which it is not: "
               "on the raw count the correlation is materially lower and falls BELOW the provenance "
               "correlation, reversing the comparison.",
     "corr_cq_rawcount_pearson":round(float(df[["citation_quality","n_cit"]].corr().iloc[0,1]),4),
     # Grounded/placeholder decomposition. Added 2026-08-11 after adversarial review found the
     # section's conclusion ("registers little about whether they are grounded") contradicted by
     # the data: citation_quality tracks the GROUNDED count strongly. The real rubric failure is
     # that placeholders are barely penalised, not that grounding is ignored.
     "corr_cq_log_grounded_pearson":round(float(np.corrcoef(
         df.citation_quality, np.log1p(df.n_grounded))[0,1]),4),
     "corr_cq_log_placeholder_pearson":round(float(np.corrcoef(
         df.citation_quality, np.log1p(df.n_placeholder))[0,1]),4),
     "cq_decomposition_fe":(lambda mm:{
         "spec":"citation_quality ~ log1p(n_grounded) + log1p(n_placeholder) + log_words + C(pattern)",
         "beta_log_grounded":round(float(mm.params["lg"]),4),
         "p_log_grounded":float(mm.pvalues["lg"]),
         "beta_log_placeholder":round(float(mm.params["lp"]),4),
         "p_log_placeholder":round(float(mm.pvalues["lp"]),4),
         "r2_adj":round(float(mm.rsquared_adj),4)})(
         smf.ols("citation_quality ~ lg + lp + log_words + C(pattern)",
                 data=df.assign(lg=np.log1p(df.n_grounded), lp=np.log1p(df.n_placeholder))).fit()),
     # PER-JUDGE placeholder coefficient. The pooled -0.0206 is CANCELLATION of opposite-signed
     # judge effects, not a small common penalty: GPT-5.2 mildly REWARDS placeholders while both
     # Claude judges penalise them. Quoting only the pooled value conceals a judge effect that is
     # the subject of finding (iii).
     "cq_placeholder_per_judge":{j:(lambda mm:{
         "beta_log_placeholder":round(float(mm.params["lp"]),4),
         "p_log_placeholder":round(float(mm.pvalues["lp"]),4),
         "beta_log_grounded":round(float(mm.params["lg"]),4)})(
         smf.ols("cq_j ~ lg + lp + log_words + C(pattern)",
                 data=df.assign(lg=np.log1p(df.n_grounded), lp=np.log1p(df.n_placeholder),
                     cq_j=df.set_index(["pattern","query_id"]).index.map(
                         sc[(sc.dimension=="citation_quality")&(sc.judge==j)]
                         .groupby(["pattern","query_id"],observed=True)["score"].mean())
                     ).dropna(subset=["cq_j"])).fit())
...
def _merge(dst,src):
    ...
    `p["citation_regression"]=out` destroyed the 30 leaves under
    citation_regression.query_fe.* written by a different script -- silently, on every run.
    Same store-degradation defect as build_n_eff.py. Overlay, never replace."""
    for k,v in src.items():
        if isinstance(v,dict) and isinstance(dst.get(k),dict): _merge(dst[k],v)
        else: dst[k]=v
    return dst

_merge(p.setdefault("citation_regression",{}),out)
...
for j in ["gpt52","claude_opus","claude_sonnet"]:
    ...
    density_per_judge[j]={"beta_density":round(float(mj.params["log_cit"]),4),
                          ...
                          "n":int(len(dj))}
_merge(p.setdefault("density_per_judge",{}),density_per_judge)
