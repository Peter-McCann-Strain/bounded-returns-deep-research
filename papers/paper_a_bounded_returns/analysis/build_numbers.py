...
Run:  ./venv/bin/python papers/paper_a_bounded_returns/analysis/build_numbers.py
Out:  papers/paper_a_bounded_returns/analysis/canonical_numbers.json
      papers/paper_a_bounded_returns/analysis/canonical_numbers.md
...
OUT = f"{ROOT}/papers/paper_a_bounded_returns/analysis"
...
# ---------- citation provenance ----------
MIN_REPORT_WORDS = 200   # threshold is stable from 200w upward

def citations():
    c = pd.read_parquet(f"{A}/df_citations.parquet")
    global _runs_cit
    _runs_cit = pd.read_parquet(f"{A}/df_runs.parquet")
    _runs_cit["pattern"] = _runs_cit["pattern"].astype(str)
    # eleven canonical base patterns only; excludes base_p11/base_p12, the post-hoc
    ...
    for pat, g in base.groupby("pattern", observed=True):
        ...
        # Denominator: reports that EXIST and are substantive. query_id.nunique()
        # over the citation frame silently drops every zero-citation report, so a
        # field named n_reports read 52 for base_p9 when 90 reports exist. Reports
        # under MIN_REPORT_WORDS are degenerate generation collapses (base_p9's
        # zero-citation reports median 67 words) and do not count as reports.
        nrep = int(((_runs_cit.pattern == pat) & _runs_cit.report_exists
                    & (_runs_cit.report_word_count >= MIN_REPORT_WORDS)).sum())
...
# Emit STRICT JSON. Python's json writes bare NaN/Infinity literals, which are
# not valid JSON and are rejected by strict parsers in other languages. Missing
# values (e.g. a judge that never scored an arm) become null, matching how
# repro/reference/paper_a_headline_numbers.json already encodes them.
def _strict(o):
...
with open(_tmp, "w") as f:
    json.dump(_strict(_existing), f, indent=1, allow_nan=False)
...
def md():
    ...
    dj=R["drjudge"]
    # DR-Judge reads reports/phase12_drjudge/, which is not part of the public
    # release (generated forests are excluded). Skip the section rather than
    # crashing the whole digest on a clean checkout.
    if "_error" not in dj:
        ...
        L.append("\n## DR-Judge\nnot recomputed: requires `reports/phase12_drjudge/`, which is not shipped in the public release.\n")
    ci=R["citations"]
    if "_error" not in ci:
        ct=ci["totals"]
