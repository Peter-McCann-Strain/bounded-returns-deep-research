...
DATA = str(PROJECT_ROOT / "data/analysis") + "/"
PAPER = str(PROJECT_ROOT / "papers/paper_a_bounded_returns") + "/"
...
def evt_analysis():
    ...
    # SUBSTRATE MISMATCH -- these two are NOT computable and are no longer emitted as numbers.
    # emax_* is built from sigma_run, the GPT-5.2 SINGLE-JUDGE run-to-run SD; canon_router_realisable
    # is the THREE-JUDGE PANEL headroom. Dividing one by the other returned 1.4873 and 1.6706 --
    # i.e. "149%" and "167%" of a quantity that is a fraction and cannot exceed 1. The correct
    # denominator would need the panel-mean run-to-run SD, which is UNMEASURABLE from this corpus:
    # no replicate run is judged by anything but gpt52. Averaging three judges cannot increase
    # run-to-run SD, so the substrate-matched fraction is bounded above by the value below and
    # below by the single-judge figure -- an interval, not a point.
    frac_gumbel_panel = None
    frac_exact_panel = None
    panel_fraction_bound = [round(frac_exact, 4), round(emax_exact / canon_router_realisable, 4)]
    ...
    verdict = (
        ...
        "a pure selection artefact; a resampling procedure retaining %.0f%% (%.4f) is reported "
        "below but is NOT a winner's-curse correction (see bootstrap_fraction_DEFECT_NOTE), "
    ...
    return {
        ...
        "bootstrap_fraction_DEFECT_NOTE": (
            "This value (0.753) is arithmetically reproducible under its stated recipe but is "
            "MISLABELLED: it is not a winner's-curse correction and must not be described as one. "
            "The procedure selects on T+noise and then evaluates on the SAME observed run that "
            "defined the selection, so selection noise and evaluation noise stay perfectly "
            "correlated and the curse is RETAINED, not removed. Demonstrated without modelling "
            "assumptions: run the identical procedure on 11 independent replicates of a SINGLE "
            "architecture (base_p0, 30 queries, gpt52), where true routing headroom is zero by "
            "construction, and it still certifies 61-69% of the raw gap as 'surviving'. A correct "
            "correction returns ~0 there. The valid decoupled design is the split-half estimator "
            "used for best-of-k in the paper (select on one criterion half, score on the held-out "
            "half); this EVT block does not use it. Do not consume this fraction as a "
            "noise-corrected quantity in downstream work."),
        ...
        "fraction_panel_headroom_explained_by_noise_gumbel": frac_gumbel_panel,
        "fraction_panel_headroom_explained_by_noise_exact": frac_exact_panel,
        "fraction_panel_headroom_bound": panel_fraction_bound,
        "fraction_panel_headroom_DEFECT_NOTE": (
            "Previously emitted 1.4873 and 1.6706 ('149%' and '167%') -- impossible for a fraction. "
            "Not a division bug: 0.09372/0.0561 = 1.6706 exactly. It is a SUBSTRATE MISMATCH between "
            "a single-judge numerator and a three-judge-panel denominator. Now emitted as null with "
            "fraction_panel_headroom_bound giving the admissible interval. Pinning it down requires "
            "replicate runs judged by the full panel, which this corpus does not contain. "
            "Do not consume the old point values in downstream work."),
...
def amdahl_analysis():
    ...
    # BASIS FIX. matched_p1 and base_p1 are 29-query (probe-set) costs, but cost_p0 above is the
    # 90-query base_p0 mean. Dividing across the two bases produced 3.2x and 12.1x -- the same
    # defect canonical_numbers.json disentanglement.budget_spread._basis_note documents:
    # "base_p0 costs 24.3% more on the 29 probe queries, so both ratios must be quoted on the
    # matched basis: 9.90x unclamped, 2.54x clamped." Use the q29 denominator for the q29 rungs.
    cost_p0_q29 = bs["base_p0_q29"]
    ladder = [
        {"label": "base_p0", "N": 1.0, "cost": cost_p0_q29, "quality": q_p0},
        {"label": "matched_p1", "N": bs["matched_p1_q29"] / cost_p0_q29,
         "cost": bs["matched_p1_q29"], "quality": q_matched},
        {"label": "base_p1", "N": bs["base_p1_q29"] / cost_p0_q29,
         "cost": bs["base_p1_q29"], "quality": q_p1},
        # base_p4 has no 29-query cost; its ratio is the documented 90-query p4_over_p0_ratio,
        # so this rung sits on a different denominator. Flagged rather than silently mixed.
        {"label": "base_p4", "N": bs["p4_over_p0_ratio"], "cost": bs["base_p4"], "quality": q_p4,
         "basis_note": "90-query basis (no 29-query cost exists for P4); other rungs are 29-query"},
    ]
