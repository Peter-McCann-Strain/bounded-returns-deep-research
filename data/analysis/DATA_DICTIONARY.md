...
This dictionary documents every column of the five frames produced by
`scripts/build_analysis_dataframes.py`: `df_queries`, `df_runs`, `df_scores`,
`df_overall_scores`, and `df_verdicts`. Six further frames ship alongside them
(`df_citations`, `df_citations_protocol_a`, `df_c0_verdicts`, `df_c0_per_report`,
`df_e14_oracle_verdicts`, `df_e14_oracle_per_report`); they are produced by
separate builders and are not column-documented here.

## Conventions

- `pattern`: e.g. `base_p4`, `ablation_p3_no_quality_eval`.
- `pattern_family`: `"base"`, `"ablation"`, `"oracle"`, `"protocol_a"`, `"variance"`, or `"disentanglement"`.
...
| pattern_family | category | `base`, `ablation`, `oracle`, `protocol_a`, `variance`, or `disentanglement` | derived |
...
| report_path | str \| null | Repository-relative path to the `.md` report when it exists, e.g. `results/experiments/<run>/<query>.md`. The `results/` tree holds generated reports and is **not** part of the public release, so these paths resolve only in a full working checkout. | filesystem |
