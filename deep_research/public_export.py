...
def export_public_tree(
    ...
    *,
    ...
    # The PII gate existed but nothing called it -- it lived only as a copy-paste line in a
    # runbook, so an export could report audit_ok while shipping a third party's identity.
    # Run it here, on the built tree, and fold its findings into the same verdict.
    pii_findings = [
        AuditFinding(
            severity="error",
            path=str(Path(path).relative_to(output_root)) if Path(path).is_relative_to(output_root) else path,
            message=f"personal data in released artefact ({n} occurrence{'s' if n != 1 else ''})",
        )
        for path, _term, n in scan_release_for_pii(output_root)
    ]
    findings = list(final_audit.findings) + pii_findings

    # Rewrite the report so the file inside the tree carries the same verdict this returns.
    report_path = _write_public_report(
        output_root,
        files=copied_rels,
        bytes_copied=bytes_copied,
    ...
    return PublicExportResult(
        ...
        bytes_copied=bytes_copied,
        audit_ok=final_audit.ok and not pii_findings,
        audit_findings=findings,
