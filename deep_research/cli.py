...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    missing_api = sorted(set(missing_generation + missing_judging))
    if not settings.has_anthropic:
        missing_api.append("ANTHROPIC_API_KEY")
    sdk_checks = {
    ...
    checks = {
        ...
        "openai_configured": not missing_api
        or all(item == "ANTHROPIC_API_KEY" for item in missing_api),
    ...
    if args.verify_api:
        live_verification = verify_api_entitlements(settings)
        checks["live_api_verification"] = live_verification
    print(json.dumps(checks, indent=2))
    if args.require_api and (missing_api or not all(sdk_checks.values())):
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    doctor.add_argument(
        "--require-api", action="store_true", help="Fail unless OpenAI and Anthropic are configured"
    )
    doctor.add_argument(
    ...
    doctor.add_argument(
        "--verify-api",
        action="store_true",
        help="Make small live provider calls to verify model and hosted-search entitlement",
    )
    ...
    reproduce.add_argument(
        "--mode",
        choices=["smoke", "reference", "api-best-effort"],
        default="smoke",
    ...
    reproduce.add_argument(
        "--limit", type=int, default=3, help="Number of public queries for non-full API mode"
    )
    reproduce.add_argument(
    ...
    cost.add_argument(
        "--limit", type=int, default=3, help="Number of public queries for non-full mode"
    )
    cost.add_argument(
    ...
    compare.add_argument(
        ...
        "--run",
        required=True,
        help="JSON summary from a comparable pattern-metric run",
    )
