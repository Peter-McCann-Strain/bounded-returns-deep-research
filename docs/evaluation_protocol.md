...
This document records the methodology used for the paper, and the code
implementing it ships with the public release: the evaluation harness under
`deep_research/evaluation/`, the ablation framework under
`deep_research/ablation/`, all 18 pattern implementations under
`deep_research/patterns/`, and the API workflow in `deep_research/cli.py`,
`deep_research/reproduce.py`, `deep_research/judge_runner.py`, and
`deep_research/api_judges.py`.

The public GitHub export does not ship the *data* produced by that code: the
`results/` tree of raw generated reports and raw judge verdict trees, or the
historical model and search snapshots. Those are excluded because they contain
third-party web content
under mixed redistribution terms, and because the model and search versions they
were produced against no longer exist. The tidy analysis frames derived from
them are shipped under `data/analysis/`, so the reported statistics remain
recomputable.

Use `README.md`, `REPRODUCIBILITY.md`, and `repro/PAPER_A_REPRO_MAP.md` for the
public command contract and the tiered reproduction boundary.
...
This evaluation protocol formalizes the methodology for comparing six automated deep research patterns (P0--P5) that share identical tool infrastructure and model access but differ in orchestration architecture. The goal is **exploratory diagnostic research**: understanding what makes deep research systems effective, what fails, where variance originates, and how architectural decisions map to quality outcomes. This understanding informs the design of next-generation research automation systems.

This is explicitly **not** hypothesis testing for a single confirmatory paper. We do not begin with a narrow hypothesis (e.g., "P4 outperforms P0") and design an experiment to reject a null. Instead, we pose eight diagnostic questions (Section 2) that decompose system quality into interpretable components, and we apply a battery of measurement methods designed to triangulate answers from multiple perspectives.
...
*This document defines the complete historical evaluation methodology. The public GitHub export intentionally ships only the release-facing API workflow, compact inputs, compact reference summaries, the final PDF, and public tests. The query corpus is defined in `data/eval_queries_v2.json`; the command-to-artifact map is in `repro/PAPER_A_REPRO_MAP.md`; human evaluation protocol is detailed in `docs/human_evaluation_protocol.md`.*
