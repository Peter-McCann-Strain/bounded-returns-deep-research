...
CONSISTENCY (hard requirement).
  * Generation backbone: gpt-4o on configured PTU deployment
...
def _assert_generation_backbone():
    """Guarantee generation uses the corpus backbone gpt-4o on PTU the configured PTU deployment."""
    ...
    # The deployment name is provisioning-specific (see GPT4O_DEPLOYMENT in
    # .env.example). Require one to be configured, and honour an explicit pin.
    _expected_deployment = os.environ.get("E5_EXPECTED_DEPLOYMENT")
    if not spec or not spec.deployment:
        raise SystemExit(
            "REFUSING: no deployment configured for gpt-4o. Set GPT4O_DEPLOYMENT "
            "to the deployment backing the E5 corpus."
        )
    if _expected_deployment and spec.deployment != _expected_deployment:
        raise SystemExit(
            f"REFUSING: gpt-4o deployment={spec.deployment!r}, "
            f"must be {_expected_deployment!r} (E5_EXPECTED_DEPLOYMENT)."
        )
...
async def amain():
    ...
    print(f"gen backbone:  gpt-4o @ the configured PTU deployment (PTU)  | search: gpt-4o-mini")
