...
1. GENERATE  reports for systems {P0, P1, P4} on the HELD-OUT items of each
   ...
   configured PTU deployment) — the SAME backbone as the corpus.  Outputs go
...
* Generation backbone is asserted to be gpt-4o / `the configured PTU deployment`.
...
def assert_generation_backbone() -> None:
    """Assert generation uses the gpt-4o corpus backbone (same as the E12 corpus).

    The deployment name is provisioning-specific. Set E12_EXPECTED_DEPLOYMENT to
    pin a particular deployment; by default this accepts whatever deployment the
    configured gpt-4o model resolves to (see GPT4O_DEPLOYMENT in .env.example).
    """
    ...
    deployment = getattr(spec, "deployment", None)
    if not deployment:
        raise SystemExit(
            "REFUSING: no deployment configured for gpt-4o. Set GPT4O_DEPLOYMENT "
            "to the deployment backing the E12 corpus."
        )
    expected = os.environ.get("E12_EXPECTED_DEPLOYMENT")
    if expected and deployment != expected:
        raise SystemExit(
            f"REFUSING: gpt-4o deployment is {deployment!r}, "
            f"expected {expected!r} (E12_EXPECTED_DEPLOYMENT)."
        )
    ...
    if os.environ.get("DR_LOCAL_LLM"):
        raise SystemExit(
            ...
            "generation uses the corpus backbone (the configured PTU deployment)."
        )
